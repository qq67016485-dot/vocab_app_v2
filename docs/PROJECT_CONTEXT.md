...
│  │  │  ├── assignment_service.py         # Word set → student assignment (per-assignment content_type)
│  │  │  ├── graphic_novel_selection_service.py # Select a GN candidate + promote its cloze
│  │  │  ├── infographic_selection_service.py   # Select an infographic candidate + promote its cloze
...
- **StudentWordSetAssignment** — FK→user, FK→WordSet, FK→assigned_by, `content_type` (`graphic_novel`/`infographic`, default `graphic_novel`). **Per-assignment content choice**: the teacher picks which instructional format this student sees for the word set (set in `AssignSetForm`, persisted by `AssignmentService.assign_word_set(content_type=...)`, which also updates existing assignments). `instructional_service.get_pack_data` serves the chosen type's selected content, falling back to the other published type then legacy stories if the chosen one isn't published.
...
- **GraphicNovel** — FK→WordPack, `candidate_index` (0..N-1), `is_selected` (Boolean), `channel` (dead column, always `'5page'`, kept pending removal), `title`, `synopsis`, `style_prompt`, `reading_level`, `metadata` (JSONField — stores `page_count` ∈ {5, 6}, derived deterministically from the pack's word count: ≤4 words → 5 pages, >4 → 6), `created_at`. `unique_together = ('pack', 'candidate_index')`. **Each pack generates `GRAPHIC_NOVEL_CANDIDATE_COUNT = 3` independent candidate novels** (each from its own full team-selection→script workflow, so they diverge in story/art/framing — countering per-call LLM variance). `is_selected` marks the one admin-chosen candidate; **no candidate is auto-selected**, so until an admin picks one the pack's graphic novel + cloze stay hidden from students (deliberate manual publish gate). Migration `0035` backfilled existing novels → `candidate_index=0, is_selected=True`.
...
- **Infographic** — FK→WordPack, `candidate_index` (0..N-1), `is_selected`, `title`, `intro_text`, `content` (JSON — `layout_mode`/`big_idea`/`visual_structure`/`scene_description`/`scene_elements[]`/`entries[]`), `style_prompt`, `reading_level`, `metadata`, single image fields mirroring GraphicNovelPage (`image`/`edited_image`/`use_edited_image`/`image_jpeg`/`edited_image_jpeg`, `display_image`/`student_image` properties), `prompt_used`, generation tracking. **Single-page alternative to the graphic novel** (migration `0036`): same 3-candidate + admin-select model (`unique_together = ('pack', 'candidate_index')`, no auto-select), neutral educational style (no Lexi Legends canon). NotebookLM-class explanatory poster, not a flashcard grid — see the infographic pipeline steps below.
- **ClozeItem** — FK→WordPack, FK→Word, nullable FK→GraphicNovel (`novel`), nullable FK→Infographic (`infographic`), `sentence_text` (with `_______` blank), `correct_answer`, `distractors`, `order`. **Cloze is per-candidate and shared across content types**: each graphic-novel OR infographic candidate generates its own staged cloze (`novel=<id>` or `infographic=<id>`). Selecting a candidate **promotes** its rows to the active set (prior active rows deleted first). **Active/promoted (student-facing) = BOTH FKs NULL**; staged = one FK set. **All active-cloze reads must filter `novel__isnull=True, infographic__isnull=True`** (instructional_service + both review endpoints + both selection services) — filtering only `novel__isnull=True` would leak staged infographic cloze to students. Active set is shared (last published wins; cloze is medium-agnostic). Migration `0036` adds the `infographic` FK.
...
- **UserWordProgress** — FK→user, FK→Word, FK→MasteryLevel, `mastery_points`, `next_review_at` (DateTimeField), `last_reviewed_at`, `learning_speed` (adaptive multiplier, default 1.0), `instructional_status` (READY/PENDING). Indexed on `(user, next_review_at)` and `(user, instructional_status, next_review_at)` (migration `0040` — serves the due-for-review query index-ordered; its prefix covers user+status filters, so the old two-column status index was dropped).
- **MasteryLevelLog** — FK→user, FK→Word, old_level, new_level, timestamp

### Questions & Practice
- **Question** — FK→Word, `question_type` (30 types), `question_text`, `options` (JSON), `correct_answers` (JSON — empty for sentence-writing types), `explanation`, `lexile_score`, `difficulty_index`, `discrimination_index`, M2M→MasteryLevel (`suitable_levels`), FK→GenerationJob. Indexed on `(word, lexile_score)` (migration `0040` — the practice picker's per-word Lexile probe).
- **PracticeSession** — FK→user, `start_time`, `end_time`
- **UserAnswer** — FK→PracticeSession, FK→user, FK→Question, `user_answer`, `is_correct`, `duration_seconds`, `answer_switches`, `answered_at`, `retry_count`, `judge_result` (JSON, nullable — the LLM verdict/error_type/hint/attempts for sentence-writing questions; null for auto-graded types). Persisted first-attempt durations and answer switches are used for response-quality scheduling baselines. Indexed on `(user, answered_at)`, `(user, is_correct)`, and `(question, answered_at)` for dashboard analytics and per-word history lookups.

### Generation Pipeline
- **GenerationJob** — FK→WordSet, FK→created_by, `job_type` (FULL_PIPELINE/QUESTIONS_ONLY/INSTRUCTIONAL_ONLY), `status` (PENDING/RUNNING/COMPLETED/FAILED/PARTIALLY_COMPLETED), `input_words` (JSON), `target_lexile`, `target_language`, `content_types` (JSON, default `['graphic_novel']`; empty/legacy = GN only — selects which instructional formats this job generates), counters (words/questions/primers/stories/graphic_novels/infographics/cloze created), `last_completed_step` (for resume), `error_message`
...
- **LLMStepConfig** — FK→`config_set`, `step_key` (one of 14 text step keys), FK→primary_site, `primary_model`, FK→fallback_site, `fallback_model`. `unique_together = (config_set, step_key)` — each set holds its own full copy of all 14 step configs. The `StepKey` enum is declared in pipeline-execution order (`word_lookup` → `translation` → `question_gen` → `primer_gen` → `pack_creation` → `gn_team_selection` → `gn_router_premises` → `gn_premise_scoring` → `gn_cloze_gen` → `gn_beat_sheet` → `gn_final_script` → `ig_design` → `ig_cloze` → `audiobook_director`); the admin step-config API returns rows in this order (not alphabetical) so the UI matrix mirrors the pipeline. `ig_design`/`ig_cloze` are the infographic substeps (seeded into all 3 sets by migration `0037`); `audiobook_director` is the on-demand voice-director call (not part of the generation pipeline; seeded by migration `0033`).
...
2. Student opens a pack → `InstructionalPackView` returns primer cards (with approved images), `story.type`, and cloze items. The served format follows the student's assignment `content_type`: it prefers the **admin-selected** candidate of that type (`is_selected=True`) and its promoted cloze (`novel__isnull=True, infographic__isnull=True`), **falling back** to the other published type, then `micro_story` (legacy), then no story. Cloze always shows only the promoted (both-FK-NULL) set.
3. Student completes the 3-step flow. `GraphicNovelReader` shows one 16:9 page image at a time (the lightweight JPEG companion, served via `student_image` to keep mobile load times low) with arrow/keyboard/swipe navigation, a sticky toolbar (title, page dots, "Done Reading" button), and a vocabulary overlay shown by default on each page. The "Done Reading" button appears with a 3-second delay on the last page to encourage reading before advancing. `InfographicReader` shows the single poster image (click-to-zoom lightbox) plus the intro text and an accessible per-word entry list. Legacy `MicroStoryView` still renders bold target words with tooltips. `InstructionalFlow` routes on `story.type`.
...
### Question Types (30 types across 9 skill categories)
| Category | Types |
|----------|-------|
...
| Sentence Production | SENTENCE_WRITE_GUIDED (L4), SENTENCE_WRITE_OPEN (L5) |

> **Defined vs. generated:** All 30 types above are defined in the `QuestionType` enum (`models.py`) and remain valid for rendering, but the current generation prompts only produce **19** of them. 17 come from `question_generation_A.txt` / `_B.txt`, and the 2 **Sentence Production** types come from the dedicated `SENTENCE_WRITE_GEN` pipeline step (`sentence_write_guided.txt` / `sentence_write_open.txt`). The 11 dormant types — every non-single-MC format: `DEFINITION_MATCHING`, `SYNONYM_MC_MULTI`, `SYNONYM_MATCHING`, `ANTONYM_MATCHING`, `CONTEXT_FILL_IN_BLANK`, `SPELLING_FILL_IN_BLANK`, `WORD_FORM_FILL_IN_BLANK`, `SENTENCE_SCRAMBLE`, `CONNOTATION_SORTING`, `COLLOCATION_FILL_IN_BLANK`, `COLLOCATION_MATCHING` — appear in no prompt and have zero rows in the DB. Treat the table as the schema catalog, not the set students actually encounter.

> **Sentence Production is the only LLM-judged, productive type.** All others are auto-graded by exact string match; sentence-writing questions have no `correct_answers` — the student writes an original sentence and an LLM judge (`sentence_evaluation_service`) verdicts it (`correct`/`almost`/`incorrect`) with a coaching hint. See `docs/feature_plan/design-sentence-writing-questions.md`.
...
**Content-type gating**: `GenerationJob.content_types` (JSON, default `['graphic_novel']`; empty/legacy = GN only) selects which instructional formats a job generates. Steps 8–9 (graphic novel) run only when `'graphic_novel'` is listed; steps 10–11 (infographic) run only when `'infographic'` is listed — `orchestrator._run_step` skips a content type's steps otherwise (via `_step_content_type` + `job_generates_content_type`). The generation wizard offers GN/infographic checkboxes; `TriggerGenerationView` accepts `content_types`. Both formats use the same 3-candidate + admin-select model.

For the multi-pack GRAPHIC_NOVEL_SCRIPT step, resume is per-pack, **per-candidate**, and per-substep: each pack generates `GRAPHIC_NOVEL_CANDIDATE_COUNT = 3` independent candidate novels, and when a job fails mid-run, resuming picks each `(pack, candidate)` up at the first substep without a COMPLETED log (reusing the prior substeps' on-disk artifacts under `cand_{i}/`) rather than restarting from team selection. `restart_graphic_novel_from_substep(job, pack_id, substep_key, words_data, candidate_index)` is the single generation engine — both fresh runs and resume delegate to it (one candidate, one pack, substep→end). Prior-substep artifacts are only trusted when their COMPLETED log exists (artifacts write before validation, so presence alone could carry failed-validation output), and that check runs before the candidate's existing novel is deleted. A candidate is skipped as complete only when its story pages, review page, page count, and staged cloze all check out; a selected (published) candidate is never deleted by a resume. Candidate persistence (novel + pages + review page + staged cloze) is one atomic transaction. The per-pack substep-restart endpoint (`restart-substep`, optional `candidate_index`) regenerates one candidate from a chosen substep and then runs the full script step across **all** packs/candidates before completing — so a candidate the original run never reached (because an earlier one failed) is filled in instead of silently left without a novel. Both substep-restart thread functions claim the job atomically on entry (a PENDING/RUNNING job is skipped), closing the race where two concurrent restarts could mutate the same records.

| Step | Name | What It Does |
|------|------|--------------|
| 1 | WORD_LOOKUP | LLM defines each word (POS, definition, example sentence). Normalizes plurals/tense. |
| 2 | DEDUP | Embedding-based deduplication (cosine similarity ≥0.92). Creates Word, WordDefinition, DefinitionEmbedding — each word's writes in one atomic transaction (embedding fetched first), so a crash never leaves a word invisible to dedup. Reuses existing words if found, snapshotting the **matched** definition (not an arbitrary sibling); resume reuses words already attached to the job's word set without re-embedding. |
| 3 | TRANSLATION | LLM translates definition_text and example_sentence to target_language. Creates Translation records. |
| 4 | QUESTION_GEN | LLM generates 15 questions per word (3 per mastery level 1-5) in batches of 2 words (`QUESTION_BATCH_SIZE`). Creates Question records with suitable_levels M2M. Idempotent per batch: questions commit as each batch succeeds, and resume/retry skips batches whose words already have questions for this job (clearing only partial batches) — no wasted LLM calls, no duplicates. A batch fails (→ normal retry) if any word came back with no questions or with a `question_type` that doesn't resolve to a mastery level — never silently persists an unservable or missing question set. |
| 5 | SENTENCE_WRITE_GEN | LLM generates 2 **productive** sentence-writing questions per word (batches of 10, `SENTENCE_WRITE_BATCH_SIZE`): `SENTENCE_WRITE_GUIDED` (L4 — scenario + sentence starter) and `SENTENCE_WRITE_OPEN` (L5 — light scenario, no starter), from two independent prompts (`sentence_write_guided.txt` / `sentence_write_open.txt`). Lexile-aware (scenario readability + a judge-leniency bar stored in the question's rubric anchors). Stores scenario→`question_text`, model sentence→`example_sentence`, rubric anchors (`intended_sense`/`acceptable_use_notes`/`sentence_starter`)→`options`, `correct_answers=[]`. Gated to L4/L5 via `QUESTION_TYPE_LEVEL`. **Guided-only floor**: when content Lexile (`_content_lexile` = target×0.85) ≤ `SENTENCE_WRITE_GUIDED_ONLY_MAX_LEXILE` (600), the OPEN variant is skipped and the GUIDED question is attached to **both** L4 and L5 `suitable_levels` — lower-proficiency readers never get unscaffolded open production but still practice productively at L5. Idempotent per word per variant on resume; a pending word the LLM dropped fails the batch (never silently skipped). These have **no closed answer** — they are LLM-judged at practice time (`sentence_evaluation_service`). |
| 6 | PRIMER_GEN | LLM generates syllable_text + kid_friendly_definition (concise 3-8 word phrase). Creates PrimerCardContent. Definitions also serve as the review page vocabulary source. |
| 7 | PACK_CREATION | LLM groups words into situation-aware packs of ~6 for downstream graphic novels (free-text `narrative_approach` per pack). Creates WordPack + WordPackItem. If existing packs are missing generated words during a rerun, validates a replacement LLM grouping before replacing the old pack structure. Sequential fallback is testing-only and is logged with a quality warning. |
| 8 | GRAPHIC_NOVEL_SCRIPT | Gemini `gemini-3.1-pro-preview` runs a 6-call planning pipeline **per pack per candidate**: team selection (coin-flip solo/dual, decides `vault_framing`), router/premises, premise scoring, cloze generation (dedicated substep using only the winning premise + vocab words), beat sheet/vocab roles, and final script/self-check. **Each pack generates `GRAPHIC_NOVEL_CANDIDATE_COUNT = 3` independent candidate novels** so an admin can later pick the best (countering per-call LLM variance); each candidate runs the full workflow independently. **Page count is derived deterministically from the pack's word count — `page_count_for_word_count(n)` returns 5 for ≤4 words and 6 for >4 (threshold `GRAPHIC_NOVEL_WORD_COUNT_PAGE_THRESHOLD = 4`).** The router and scorer prompts are told this `required_page_count` so premises are designed for it, but the pipeline then forces `winning_premise['page_count'] = required_page_count` regardless of the LLM's output. The beat-sheet and final-script substeps dispatch to the matching prompt template via `GRAPHIC_NOVEL_BEAT_SHEET_TEMPLATES` / `GRAPHIC_NOVEL_SCRIPT_TEMPLATES` (`{5: 'graphic_novel_beat_sheet', 6: 'graphic_novel_beat_sheet_6page'}` and the same for script). For each candidate, one `GraphicNovel` is created (`candidate_index` 0-2, `is_selected=False`), with N story `GraphicNovelPage` rows (N = forced page_count) plus 1 review page, and per-candidate **staged** `ClozeItem` rows (`novel=<id>`). `novel.metadata['page_count']` records the length. Intermediate JSON artifacts are written under `temp/generation_artifacts/job_<id>/pack_<id>_<slug>/cand_<i>/`. **No candidate is auto-selected** — students see nothing until an admin selects one (see Candidate Selection below). Vocab integration uses the **Lexi Mini system**: 0–1 Mini summon per story max — when a hero writes a vocab word with their tool (Hugo's carpenter's pencil, Leo's wax crayon, Amara's quill, Mei's marker), it hatches a temporary creature that physically acts out the definition. Most vocab integrates through dialogue/narration/world logic without summoning a Mini. Each vocab item must declare a `pedagogical_anchor` (`demonstrated_action`, `near_synonym`, `category_example`, or `visible_referent`) so ESL learners can derive meaning from the page. Plot complexity caps scale by length via `max_locations_for_page_count(page_count)` (≤2 for 5-page, ≤3 for 6-page) and ≤2 secondary characters with a single problem thread. |
| 9 | GRAPHIC_NOVEL_IMAGES | OpenAI GPT-Image-2 generates one 1792x1024 landscape image per `GraphicNovelPage` **across all candidate novels** (so the admin compares fully-rendered candidates), including each vocabulary review page. Length-agnostic — iterates the saved page rows ordered by `novel_id`, resetting the cross-page continuity reference at each novel boundary. Each page tracks `PENDING`/`RUNNING`/`COMPLETED`/`FAILED`, attempts, and errors. The step saves successful pages (and a smaller JPEG companion in `image_jpeg` for students, best-effort), fails if any page fails or remains incomplete, and Resume retries only missing/failed pages (also backfilling any missing JPEG on skipped pages). **Post-generation, admins can refine any single page from the review screen via the edit-image endpoint (uses the current image as a reference) and toggle between the original and edited variant — see Admin Image Editing below.** |
| 10 | INFOGRAPHIC_DESIGN | (Only when `'infographic'` ∈ `job.content_types`.) Per pack, generates `INFOGRAPHIC_CANDIDATE_COUNT = 3` candidate infographics via 2 LLM substeps each — `ig_design` (the design LLM acts as art director: picks a `layout_mode` — `panorama` (one continuous landscape/map with a visual spine for sequential packs) or `gallery` (vignettes in a creative framing device for disjointed packs); emits a single-line `title` naming the big idea (no "Main Idea: Subtitle" subtitle — `_clean_infographic_title` strips a trailing generic tag like "A Vocabulary Guide" on persist), an `intro_text` hook (the short paragraph shown above the poster — must **naturally use every target word in context**, validator rejects any omitted word; stem-tolerant), `big_idea`, `visual_structure`, `scene_description`, ordered `scene_elements[]` with captions that **use** each vocab word in a sequential narrative beat (never `word: definition` — validator rejects the glossary format), and per-word `entries[]` for the study panel; also emits `color_palette` (3–5 specific vibrant colors, varied per poster) and `style_prompt` — an art-directed rendering look the LLM composes by **mixing and matching** a *distinct* combination of modern editorial-infographic terms (modern flat-vector illustration / clean diagrammatic design / data-visualization explainer look; vibrant cohesive palette / soft gradients / confident editorial color blocking; clean crisp outlines / simplified iconographic figures / labeled callouts / banners-ribbons / floating text boxes) per candidate, so the 3 candidates diverge in look rather than sharing one style; the design is explicitly a modern editorial infographic, **not** a children's storybook/cartoon illustration; `style_prompt` flows into `infographic_image.txt` where `STYLE_LOCK` is only a thin shared floor that must not override it) → `ig_cloze` (premise-free staged cloze). Creates one `Infographic` per candidate (`is_selected=False`) + staged `ClozeItem` rows (`infographic=<id>`). Artifacts under `temp/generation_artifacts/job_<id>/pack_<id>_<slug>/infographic_cand_<i>/`. Engine: `restart_infographic_from_substep(...)`. **No candidate auto-selected** — admin picks one (see Candidate Selection). Neutral educational style, no Lexi Legends canon. |
| 11 | INFOGRAPHIC_IMAGE | (Only when `'infographic'` ∈ `job.content_types`.) OpenAI GPT-Image-2 renders one 1792x1024 poster per candidate `Infographic` (so the admin compares all 3), best-effort JPEG companion in `image_jpeg`. `build_infographic_image_prompt` branches the image template on `layout_mode` (spine vs framing-device language). **Target-word emphasis**: each scene-element caption has its target vocab words wrapped in `**markers**` by `_mark_vocab_terms` (case-insensitive, word-boundary, suffix-tolerant for plurals/`-ed`/`-ing`, no double-wrap — replaces the old naive `str.replace`), and the template (plus an explicit `{vocab_words}` list from `_all_target_terms`) instructs the model to render each marked word in a **bolder weight + a distinct accent color** so the taught word stands out in its caption (without printing the asterisks). Continue-on-failure per candidate; failed candidates marked FAILED. |

`STORY_CLOZE_GEN` remains a valid `GenerationJobLog.Step` enum for historical logs, but it is legacy-only and is not part of `PIPELINE_STEP_ORDER` or active restart targets. `GN_6PAGE_SCRIPT` and `GN_6PAGE_IMAGES` likewise stay in the enum so old log rows remain readable, but no current code path emits them. `INFOGRAPHIC_DESIGN` / `INFOGRAPHIC_IMAGE` are appended to `PIPELINE_STEP_ORDER` after the GN steps. Existing `MicroStory` records still render through the student fallback.

**LLM models used:** Configurable per-step via `LLMStepConfig` (admin UI). Default seed: Gemini `gemini-3.1-pro-preview` primary / `gemini-3-pro-preview` fallback for all 14 text step keys (the `gn_cloze_gen` substep was seeded by migration 0026; the on-demand `audiobook_director` voice-director step by migration 0033; the `sentence_write_gen` generation step + `sentence_judge` answer-time judge by migration 0039, both cloned from `question_gen`). OpenAI GPT-Image-2 generates graphic novel page images in step 9 (not configurable). Qwen3-Embedding-8B via SiliconFlow handles step 2 embeddings. The `llm_config_service.get_step_config(step_key)` cache layer resolves site+model at runtime; `_call_llm_with_config()` routes by provider type (`gemini_native` / `openai_compatible` / `anthropic`).

**Prompt templates** are in `vocabulary/prompts/`: `word_lookup.txt`, `question_generation_A.txt`, `question_generation_B.txt`, `sentence_write_guided.txt`, `sentence_write_open.txt`, `sentence_judge.txt` (answer-time productive-sentence judge), `translation.txt`, `pack_grouping.txt`, `primer_generation.txt`, `graphic_novel_team_selector.txt`, `graphic_novel_router.txt`, `graphic_novel_premise_scorer.txt`, `graphic_novel_beat_sheet.txt`, `graphic_novel_beat_sheet_6page.txt`, `graphic_novel_script.txt`, `graphic_novel_script_6page.txt`, `graphic_novel_page.txt`, `graphic_novel_review_page.txt`, `audiobook_voice_director.txt` (read-along voice director), the infographic set (`infographic_design.txt`, `infographic_cloze.txt`, `infographic_image.txt`), and `story_cloze_generation.txt` (legacy-only). The two `_6page` files are dispatched by the forced `page_count` (derived from the pack's word count), not by a separate channel.
...
**Admin status polling:** `GenerationJobStatus` polls `/api/generation-jobs/<id>/` and `/api/generation-jobs/<id>/logs/` every 30 seconds. Per-step status is derived from the authoritative `job.status` + `last_completed_step` (the active step is the one right after `last_completed_step`), not from the latest per-step log — so a step that is mid-retry renders RUNNING with a "Retrying" detail (from the retry marker's `retry_message`) instead of a false FAILED. The status and logs views display the `Graphic Novel Script` and `Graphic Novel Images` step labels plus per-page image status from `graphic_novel_image_pages`. Because every pack now runs `GRAPHIC_NOVEL_CANDIDATE_COUNT = 3` candidates, both progress views group **by pack → by candidate** rather than flat lists: the "Planning Substeps" accordion (`renderCandidateSubsteps`) nests each candidate's 6 substeps under its own collapsible "Candidate N" header (completed candidates auto-collapse, failed ones get a red border and stay open; the per-substep restart ↻ passes `candidate_index`), and the image-page progress (`GraphicNovelImageProgress`) shows each candidate as a single row with a `done/total` count and one small status chip per page (hover for page number/status/error). Both rely on `candidate_index` being present in the backend payloads — substep logs already carry it in `output_data`, and `_graphic_novel_image_page_statuses` now includes it per page. The **Infographic Design** step gets the same per-candidate substep accordion: its `design` → `cloze` substeps both log under the single `INFOGRAPHIC_DESIGN` step, so without this the row would only show the last-logged (cloze) substep. The accordion is driven by a new `infographic_design_substeps` payload field (`_substep_statuses_for_step` is the generic serializer shared with the GN step) and has its own per-substep restart (↻) posting to `restart-infographic-substep` (the GN one posts to `restart-substep`; `renderSubstepAccordion` takes the endpoint as a `restartEndpoint` arg). If a job stalls for 15 minutes with no log activity, the status endpoint marks the job FAILED, records a FAILED log for the active step, resets the word set out of `GENERATING`, and marks any RUNNING graphic novel page FAILED. Resume then restarts from `GRAPHIC_NOVEL_IMAGES` when `last_completed_step` is `GRAPHIC_NOVEL_SCRIPT`; completed pages are skipped and only missing/failed pages are retried. Jobs whose `last_completed_step` is the now-removed `GN_6PAGE_*` value are treated as fully complete by `resume_pipeline`.
...
**Candidate selection & publish gate:** Each pack generates 3 candidate graphic novels (`candidate_index` 0-2), none auto-selected. An admin reviews all candidates on the Generation Review screen and picks one via `POST /api/graphic-novels/<novel_id>/select/` (`SelectGraphicNovelCandidateView` → `services/graphic_novel_selection_service.select_graphic_novel_candidate`). Selecting a candidate (1) sets `is_selected=True` on it and clears the flag on its siblings, and (2) **promotes** its staged cloze (`ClozeItem` with `novel=<id>`) into the pack's active set (`novel=None`, deleting the prior promoted rows first). It is idempotent and reversible — re-selecting a different candidate re-publishes that one and re-promotes its cloze. Losing candidates are kept (hidden from students). **Until an admin selects a candidate, the pack's graphic novel and cloze are invisible to students** (`instructional_service` reads `graphic_novels.filter(is_selected=True)` and `cloze_items.filter(novel__isnull=True)`) — a deliberate manual publish gate. Read-along audio stays manual: the admin only triggers `generate-audio` on the selected novel. The two admin **review** content endpoints (`GenerationJobContentView`, `WordSetContentView`) return `graphic_novels` (an array of all candidates, each with its own staged cloze) plus `graphic_novel` (the selected candidate, or `null`). Frontend: `GenerationReview.jsx` lays the candidates out as a **compare strip + detail** (`PackGraphicNovels`): a compact `CandidateStripRow` per candidate (label, title, lexile/page count, a horizontal band of page thumbnails, and a Select button — the selected one outlined green, the focused one highlighted) sits above a full-width `CandidateDetail` panel that renders the focused candidate's large page grid (280px min columns to use the wider portal) with per-page image edit/redraw + audio controls and the candidate's cloze. Clicking a strip row focuses it; focus defaults to the selected candidate (else the first). The teacher portal shell was widened (`teacher.css` `.t-shell` `max-width` 1400px → 1760px) to give this view more horizontal room.

**Infographic candidate selection** mirrors this exactly for the infographic content type: `POST /api/infographics/<infographic_id>/select/` (`SelectInfographicCandidateView` → `services/infographic_selection_service.select_infographic_candidate`) sets `is_selected`, clears siblings, and promotes its staged cloze (`ClozeItem` with `infographic=<id>`) into the pack's active set (**both** `novel` and `infographic` NULL, deleting prior active rows first). The review endpoints also return `infographics` (array of candidates, each with its staged cloze + scene fields) plus `infographic` (selected/null); `GenerationReview.jsx` renders them via `PackInfographics` (thumbnail compare strip + detail panel with a Select button). The active cloze set is **shared** across both content types — selecting either format re-promotes its cloze, so the last published format owns the active set (fine, since cloze is medium-agnostic).
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/instructional/packs/<pack_id>/` | Full pack data: primer cards (with images), `story.type` discriminator (`graphic_novel` pages, `infographic` poster, or `micro_story` fallback — chosen by the student's assignment `content_type`), cloze items |
...
| POST | `/api/word-sets/<id>/assign/` | Assign to students/groups. Optional `content_type` (`graphic_novel`/`infographic`, default GN) sets which instructional format the assigned students see. Rejected (400) if the set has no admin-selected content, or if `content_type` names a format that isn't published for the set |
| GET | `/api/word-sets/<id>/assignments/` | Current assignments for the set: `student_ids`, `group_ids`, prefilled `content_type`, and `available_content_types` (the formats with an admin-selected candidate — drives which options `AssignSetForm` shows) |
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/word-sets/<id>/generate/` | Start full pipeline. Optional `content_types` (array of `graphic_novel`/`infographic`, default `['graphic_novel']`) selects which formats to generate |
...
| GET | `/api/generation-jobs/<id>/content/` | Content generated by this job. For each pack: `graphic_novels` (array of all candidate novels, each with pages/images/status + its staged cloze) and `graphic_novel` (the selected candidate, or null); plus `infographics` (array of all candidate infographics, each with image/scene fields + its staged cloze) and `infographic` (the selected one, or null) |
...
| POST | `/api/generation-jobs/<id>/restart-substep/` | Restart a graphic novel script substep for one pack candidate (`pack_id` + `substep` + optional `candidate_index`, default 0), then fill any pack/candidate that never got a novel |
| POST | `/api/generation-jobs/<id>/restart-infographic-substep/` | Restart an infographic design substep (`substep` ∈ {`design`,`cloze`}) for one pack candidate (`pack_id` + optional `candidate_index`), then fill any candidate that never got an infographic. Daemon thread; does NOT re-render the poster image. **409** if a job is already running |
...
| POST | `/api/graphic-novels/<novel_id>/select/` | Select this candidate as the pack's published graphic novel: sets `is_selected`, clears siblings, and promotes its staged cloze to the pack's active set. Idempotent + reversible. Until a candidate is selected, the pack stays hidden from students. Synchronous. |
| POST | `/api/infographics/<infographic_id>/select/` | Select this candidate as the pack's published infographic: sets `is_selected`, clears siblings, and promotes its staged cloze to the pack's active set (both-FK-NULL). Mirrors the graphic-novel select; idempotent + reversible. Synchronous. |
...
- Tests: `pytest` (backend, with pytest-django + factory-boy). `backend/tests/conftest.py` has an autouse fixture that clears Django's cache around every test — the project has no `CACHES` setting, so Django uses a process-global `LocMemCache` that pytest-django does NOT roll back between tests; without the clear, cache-backed state (notably `llm_config_service`, key `llm_step_configs_all`) leaks from one test into the next and produces cascading "passes alone, fails in suite" failures.
