...
- **ClozeItem** — FK→WordPack, FK→Word, nullable FK→GraphicNovel (`novel`), `sentence_text` (with `_______` blank), `correct_answer`, `distractors`, `order`. **Cloze is per-candidate**: each candidate novel generates its own cloze, stored staged as `novel=<id>`. Selecting a candidate **promotes** its rows to `novel=None` (the active set students practice; prior promoted rows are deleted first). `novel=None` = promoted/active (student-facing); `novel=<id>` = staged. Student + practice reads filter `novel__isnull=True`.
...
| GET | `/api/generation-jobs/<id>/content/` | Content generated by this job. For each pack: `graphic_novels` (array of all candidate novels, each with pages/images/status + its staged cloze) and `graphic_novel` (the selected candidate, or null) |
...
2. Student opens a pack → `InstructionalPackView` returns primer cards (with approved images), `story.type`, and cloze items. New packs prefer the **admin-selected** `graphic_novel` candidate (`is_selected=True`) and its promoted cloze (`novel__isnull=True`); if no candidate has been selected yet, the graphic novel is absent and the pack falls back to `micro_story` (legacy) or shows no story. Cloze likewise shows only the promoted set.
