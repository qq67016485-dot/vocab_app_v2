...
│  │  ├── services/
│  │  │  ├── generation/                    # 10-step AI content pipeline (modular package)
...
│  │  │  │  ├── step_infographic.py       # Steps 9-10: infographic design+cloze+image (3 candidates)
...
> **Defined vs. generated:** All 28 types above are defined in the `QuestionType` enum (`models.py`) and remain valid for rendering, but the current generation prompts (`question_generation_A.txt` / `_B.txt`) only produce **17** of them. The 11 dormant types — every non-single-MC format: `DEFINITION_MATCHING`, `SYNONYM_MC_MULTI`, `SYNONYM_MATCHING`, `ANTONYM_MATCHING`, `CONTEXT_FILL_IN_BLANK`, `SPELLING_FILL_IN_BLANK`, `WORD_FORM_FILL_IN_BLANK`, `SENTENCE_SCRAMBLE`, `CONNOTATION_SORTING`, `COLLOCATION_FILL_IN_BLANK`, `COLLOCATION_MATCHING` — appear in no prompt and have zero rows in the DB. Treat the table as the schema catalog, not the set students actually encounter.
...
## 10-Step AI Content Generation Pipeline
