...
- **Admin per-page image editing (2026-05-31).** Admins can refine any single page image post-generation via `POST /api/graphic-novel-pages/<id>/edit-image/` (uses the current image as reference; result in `edited_image`, original preserved) and flip variants via `select-image/`. All read paths use the `display_image` property.
