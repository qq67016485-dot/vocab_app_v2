...
`requirements.txt` lists `pymysql` + `anthropic` + `google-genai` — do NOT add `mysqlclient` (requires C compiler, not used).
...
- Read-along audiobook (phase 1, on-demand): `GraphicNovelPageAudio` (OneToOne→`GraphicNovelPage`, migration `0032`) stores **one stitched WAV per page**. Service package `services/audiobook/`: `events.build_page_events` (pure — `panel_descriptions` → ordered speech events, one narration box / dialogue line each, with pauses), `voices.voice_for` (stable hero map Leo=Puck/Amara=Kore/Mei=Fenrir/Hugo=Aoede; narrator by `age_band`; supporting chars hashed into a pool), `tts_client.synthesize` (isolated **native** `genai.Client` — the `GEMINI_BASE_URL` OpenAI-proxy cannot do audio modality, so TTS always uses the native client + `GEMINI_API_KEY`; parses sample rate from the response mime type; retries once), `stitch` (stdlib `wave` only — no ffmpeg/pydub on the box), `generator.generate_novel_audio` (continue-on-failure per page; skips review page + already-COMPLETED unless `regenerate`). Model: `gemini-2.5-pro-preview-tts`. Spoken text comes verbatim from `panel_descriptions` (no OCR). API (admin, async like edit-image): `POST /api/graphic-novels/{id}/generate-audio/` → 202 + daemon thread (409 if running), `GET .../audio-status/` poll, `POST /api/graphic-novel-pages/{id}/regenerate-audio/`. Student payload (`instructional_service`) carries `audio_url`. UI: button + poller in `GenerationReview.jsx`, `<audio>` per page in `GraphicNovelPageEditor.jsx`. Deferred: per-event timing/highlight sync, MP3/Opus, ambience/SFX. Production rules: `docs/feature_plan/lexi-legends-audiobook-production-bible.md`
