...
│   │   │   └── generation_views.py # AI pipeline: trigger, status, review, auto-approve
│   │   ├── services/
...
- **GeneratedImage** — FK→Word, `image` (ImageField), `prompt_used`, `status` (APPROVED by default in the pipeline; PENDING_REVIEW/REJECTED retained for legacy/admin states)
...
| 5 | PACK_CREATION | LLM groups words into thematic packs of ~5. Creates WordPack + WordPackItem. On resume, adds any unpacked generated words to existing packs. |
...
| 8 | IMAGE_GEN | Gemini generates one image per word. Creates GeneratedImage with APPROVED status automatically. Continues on individual failures. |
