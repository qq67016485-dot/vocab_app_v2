...
def restart_graphic_novel_from_substep(job, pack_id, substep_key, words_data,
    ...
    # resume skip-guard in _step_graphic_novel_script already leaves selected
    # candidates alone; this is the backstop for the manual restart path.
    existing_novel = pack.graphic_novels.filter(candidate_index=candidate_index).first()
    if existing_novel and existing_novel.is_selected:
        raise ValueError(
            f"Cannot restart pack '{pack.label}' candidate {candidate_index} "
            f"from '{substep_key}': the candidate is published (is_selected). "
    ...
    # resume (a later failure would look complete). Drop the rows for every
    # substep being rerun before regenerating them.
    rerun_keys = set(substep_keys[start_idx:])
    stale_log_ids = [
        log.id
        for log in job.logs.filter(step=GenerationJobLog.Step.GRAPHIC_NOVEL_SCRIPT)
        if isinstance(log.output_data, dict)
        and log.output_data.get('pack_id') == pack.id
        and log.output_data.get('candidate_index', 0) == candidate_index
    ...
    if start_idx <= 0:
        ...
        team_result, _ = _run_graphic_novel_substep(
            ...
            ctx=SubstepContext(team_options=team_options),
            candidate_index=candidate_index,
        )
        novel_metadata, lexi_context, router_lexi_context = _build_team_contexts(
            team_result, content_lexile,
    ...
    # Persist the beat sheet so image prompts can enrich a page's synopsis with
    # its beat (_format_synopsis_for_page reads metadata['beat_sheet']).
    novel_metadata['beat_sheet'] = beat_result.get('beat_sheet', [])
