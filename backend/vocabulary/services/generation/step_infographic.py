...
def _run_infographic_substep(job, pack, substep_config, site_config, system_prompt,
                             ...
                             fallback_site_config=None):
    substep = substep_config['key']
    label = substep_config['label']
    template_name = substep_config['template']
    last_exc = None

    # Mirror the word-level steps' [primary ×3, fallback ×1] attempt plan: the
    # configured fallback site gets one final attempt after the primary site's
    # retries are exhausted (previously the fallback was never used, so a down
    # primary hard-failed the step despite a configured fallback).
    attempt_configs = [site_config] * (1 + max_retries)
    if fallback_site_config:
        attempt_configs.append(fallback_site_config)

    for attempt, attempt_config in enumerate(attempt_configs):
        _log_infographic_substep(
            ...
            model=attempt_config['model'], prompt_template=template_name,
            prompt_text=system_prompt, candidate_index=candidate_index,
        )
        start = time.time()
        artifact_path = ''
        try:
            response = _call_llm_with_config(attempt_config, system_prompt, user_prompt)
            artifact_path = _write_infographic_artifact(
                job, pack, substep, substep_config['filename'],
                attempt_config['model'], input_summary, response, candidate_index,
            )
            if validator:
            ...
            _log_infographic_substep(
                ...
                error_message=str(exc), model=attempt_config['model'],
...
def restart_infographic_from_substep(job, pack_id, substep_key, words_data,
    ...
    # A published candidate is never regenerated by a substep restart — the
    # resume skip-guard in _step_infographic_design already leaves selected
    # candidates alone; this is the backstop for the manual restart path.
    existing_ig = pack.infographics.filter(candidate_index=candidate_index).first()
    if existing_ig and existing_ig.is_selected:
        raise ValueError(
            f"Cannot restart pack '{pack.label}' infographic candidate "
            f"{candidate_index} from '{substep_key}': the candidate is "
            "published (is_selected). Select a different candidate first."
        )
    ...
    # Substep logs are append-only, so COMPLETED rows from an earlier run of
    # this candidate survive a restart and would defeat the log-authoritative
    # resume. Drop the rows for every substep being rerun (a restart from
    # 'cloze' keeps the design log; a restart from 'design' drops both).
    rerun_keys = set(substep_keys[start_idx:])
    stale_log_ids = [
        log.id
        for log in job.logs.filter(step=GenerationJobLog.Step.INFOGRAPHIC_DESIGN)
        if isinstance(log.output_data, dict)
        and log.output_data.get('pack_id') == pack.id
        and log.output_data.get('candidate_index', 0) == candidate_index
        and log.output_data.get('substep') in rerun_keys
    ]
    if stale_log_ids:
        GenerationJobLog.objects.filter(id__in=stale_log_ids).delete()
    ...
    if start_idx <= 0:
        ...
        design_config = get_step_config(INFOGRAPHIC_SUBSTEPS[0]['config_key'])
        design_result, _ = _run_infographic_substep(
            job, pack, INFOGRAPHIC_SUBSTEPS[0],
            design_config['primary'],
            design_template,
            ...
            candidate_index=candidate_index,
            fallback_site_config=design_config['fallback'],
        )

    cloze_template = _llm_service.load_prompt_template(INFOGRAPHIC_SUBSTEPS[1]['template'])
    cloze_config = get_step_config(INFOGRAPHIC_SUBSTEPS[1]['config_key'])
    ...
    cloze_result, _ = _run_infographic_substep(
        ...
        ctx=SubstepContext.from_input_summary(input_summary),
        candidate_index=candidate_index,
        fallback_site_config=cloze_config['fallback'],
    )
...
def _term_in_text(term, text):
    ...
    the tolerance the caption check uses. Stemming is symmetric: the term's
    stem is also compared against each text token's stem (prefix match either
    way), so "batteries" matches "battery" and vice versa.
    """
    term = (term or '').strip()
    if not term:
        return True
    ...
    if stem in text:
        return True
    return any(
        token_stem.startswith(stem) or stem.startswith(token_stem)
        for token_stem in (_term_stem(tok) for tok in re.findall(r'[a-z0-9]+', text))
        if token_stem
    )
...
def _validate_infographic_design_result(result, ctx=None):
    ...
    # layout_mode selects the image-prompt guidance block — an invalid value
    # would silently render with panorama guidance, so fail loud and retry.
    layout_mode = (result.get('layout_mode') or '').strip().lower()
    if layout_mode not in ('panorama', 'gallery'):
        raise ValueError(
            "Infographic design must return 'layout_mode' of 'panorama' or "
            f"'gallery'; got {result.get('layout_mode')!r}."
        )
    ...
    for idx, el in enumerate(scene_elements):
        ...
        # flashcard-looking posters. Word-boundary matching: ':' and '—' may
        # follow the term directly, but '-' requires surrounding whitespace so
        # hyphenated compounds ("the art-room glowed") don't false-fail.
        caption = el.get('caption', '')
        for term in (el.get('vocab_terms') or []):
            if not term:
                continue
            glossary_re = re.compile(
                rf"\b{re.escape(term.strip())}\s*[:—]"
                rf"|\b{re.escape(term.strip())}\s+-\s",
                re.IGNORECASE,
            )
            if glossary_re.search(caption):
                raise ValueError(
                    f"Scene element {idx} caption uses a definition format "
                    f"('{term}: …') for '{term}'. Captions must USE the word "
