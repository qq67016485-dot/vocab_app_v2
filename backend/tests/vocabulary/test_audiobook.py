...
class TestRegeneratePageAudioView:
    def test_returns_202(self, admin_client, novel_with_pages):
        _, pages = novel_with_pages
        url = reverse(
            'graphic-novel-page-regenerate-audio', kwargs={'page_id': pages[0].id},
        )
        with patch('vocabulary.views.generation_views._run_page_audio'):
            resp = admin_client.post(url, {}, format='json')
        assert resp.status_code == status.HTTP_202_ACCEPTED
        assert resp.data['page_id'] == pages[0].id

    def test_404_for_missing_page(self, admin_client, db):
        url = reverse(
            'graphic-novel-page-regenerate-audio', kwargs={'page_id': 99999},
        )
        resp = admin_client.post(url, {}, format='json')
        assert resp.status_code == status.HTTP_404_NOT_FOUND

    def test_409_when_page_running(self, admin_client, novel_with_pages):
        _, pages = novel_with_pages
        audio, _ = GraphicNovelPageAudio.objects.get_or_create(page=pages[0])
        audio.status = GraphicNovelPageAudio.Status.RUNNING
        audio.save()
    ...
    @patch('vocabulary.services.audiobook.voice_director.direct_novel',
           return_value={'character_profiles': {}, 'directed_index': {}})
    def test_worker_regenerates_one_page_only(
        self, _direct, _mock, novel_with_pages,
    ):
        """The per-page worker fills a single failed/missing page without
        touching its siblings — the gap-fill use case."""
        from vocabulary.views.generation_views import _run_page_audio
        _, pages = novel_with_pages
        target = pages[1]
        _run_page_audio(target.id)
        audio = GraphicNovelPageAudio.objects.get(page=target)
        assert audio.status == GraphicNovelPageAudio.Status.COMPLETED
        assert audio.audio
        # Sibling pages were not generated by the per-page worker.
        assert not GraphicNovelPageAudio.objects.filter(page=pages[0]).exists()
        assert not GraphicNovelPageAudio.objects.filter(page=pages[2]).exists()
